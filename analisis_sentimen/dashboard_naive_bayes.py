...
# -------------------------------------------------------------------------------------------------
nb_all = GaussianNB()
nb_all.fit(X_train_TFIDF, y_train)

y_pred_all = nb_all.predict(X_test_TFIDF)
...
# -------------------------------------------------------------------------------------------------
# chi square 1992 fitur
chi2_1992_features = SelectKBest(chi2, k=1992)
X_1992_best_features = chi2_1992_features.fit_transform(train_tf_idf, y_train)

nb1992 = GaussianNB()
nb1992.fit(X_1992_best_features, y_train)

X_test_1992_chi2 = chi2_1992_features.transform(X_test_TFIDF)

y_pred_1992 = nb1992.predict(X_test_1992_chi2)

kolom_1992 = ['negatif', 'positif']
confm_1992 = confusion_matrix(y_test, y_pred_1992)
df_cm_1992 = pd.DataFrame(confm_1992, index = kolom_1992, columns = kolom_1992)
...
if menu == 'Introduction' :
    ...
    if selected_option == options[1] :
        ...
        with st.container() :
            ...
            st.image(f"{script_dir}/image/naive_bayes_plot.png", caption = "Plot Eksperimen k-feature", use_column_width=True)
        ...
        # Base on Accuracy
        with col1 :
            st.metric(value = f'1992 Features', label = '**Base on Accuracy**')

        # Base on F1-Score
        with col2 :
            st.metric(value = '1992 Features', label = '**Base on F1-Score**')

        with st.container() :
            fig, ax = plt.subplots()
    
            st.markdown("<h4 style='text-align: left;'>Confusion Matrix</h4>", unsafe_allow_html=True)
            sns.heatmap(df_cm_1992, cmap = 'Greens', annot=True, fmt=".0f", ax = ax)
            ax.set_xlabel('Sentimen Sebenarnya')
            ax.set_ylabel('Sentimen Prediksi')
            ...
            st.markdown("<h4 style='text-align: left;'>Classification Report</h4>", unsafe_allow_html=True)
            st.text(classification_report(y_test, y_pred_1992))
